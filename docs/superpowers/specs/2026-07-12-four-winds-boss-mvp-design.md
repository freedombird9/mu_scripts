...
「1350级怪物」当前不在可读页面中，节点名称与路径尚未现场验证。实现必须将该入口置于功能开关之后：扫描不到已验证的入口时，`TRAVEL_FARM` 只能记录 `farm_target_missing` 并进入安全等待，不能猜测节点或点击其他地图项。用户稍后进入对应地图并完成只读 FGUI 采集后，才允许启用该动作。
